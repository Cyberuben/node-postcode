...
class PostcodeClient {
    constructor(options) {
        ...
        if (!options.secret)
            throw new TypeError("'options.secret' has to be set");
    ...
    address(postcode, number, numberAddition) {
        ...
        if (!/^[1-9][0-9]{3}[A-Z]{2}$/.test(postcode))
            throw new TypeError("'postcode' is not in a valid format");
        ...
        if (number) {
            if (
                ...
                throw new TypeError(
                    "'number' is not a valid number or out of range"
                );
            }
        } else {
    ...
    signal(options) {
        if (!options || typeof options != 'object')
            throw new TypeError(
                "'options' is required and should be an object"
            );
        if (Object.keys(options).length == 0)
            throw new TypeError("'options' must have at least one option");

        return new Promise((resolve, reject) => {
            this._r
                .post('/signal/check', options)
                .then(res => {
                    resolve(res);
                })
                .catch(err => {
